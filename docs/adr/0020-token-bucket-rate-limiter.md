...
- **rate limit / token bucket (요청 제한·토큰 양동이)** — 정해진 속도보다 빨리 들어오는 요청을 막는 것. 양동이에 토큰이 일정 속도로 채워지고 요청 1건이 1개를 쓴다. 비면 거절하되, 가득 찬 만큼은 한꺼번에(burst) 몰아 쓸 수 있다.
- **burst (버스트)** — 평소엔 조용하다가 잠깐 몰아서 보내는 폭주. 양동이가 가득 찬 만큼은 burst 를 허용해 정상 사용자의 갑작스런 연타도 막지 않는다.
- **Lua 원자 스크립트 (atomic)** — Redis 에서 '읽고→계산하고→쓰기'를 중간에 누구도 못 끼어들게 한 덩어리로 실행하는 작은 프로그램. 동시에 온 요청이 토큰을 같이 빼가 숫자가 꼬이는 걸 막는다.
- **fail-open (열어둔 채 실패)** — Redis 가 죽으면 막지 말고 통과시키는 정책. 발매 폭주에 Redis 까지 죽었을 때 사용자 전체를 막아 사고를 키우지 않으려는 선택(정합성은 DB 락이 따로 지킴).
