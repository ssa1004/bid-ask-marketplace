...
- **cache invalidation (캐시 무효화)** — 데이터가 바뀌면 캐시에 남은 옛 값을 '이거 이제 못 써' 하고 비우는 것. 안 비우면 사용자가 한동안 옛 시세를 본다.
- **pub/sub (발행-구독)** — 한 서버가 채널에 '발행'하면, 그 채널을 '구독'한 모든 서버가 알아서 받는 우체통 방식. "나 sku-X 바꿨으니 너희도 비워" 신호를 모든 pod 에 뿌리는 데 쓴다.
- **at-most-once / fire-and-forget (최대 한 번·쏘고 잊기)** — 메시지가 많아야 한 번 도착하고 못 받아도 다시 안 보내는 약한 보장. 받는 쪽이 잠깐 끊겨 있으면 유실될 수 있어, 짧은 TTL 을 안전망으로 둔다.
- **round-trip (자기 메시지 되돌이)** — 자기가 보낸 신호를 자기가 또 받아 쓸데없이 반복하는 것. 메시지에 보낸 pod 이름을 박아 자기 것은 무시한다.
