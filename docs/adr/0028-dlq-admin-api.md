...
- **DLQ (Dead Letter Queue, 죽은 편지함)** — 아무리 재시도해도 처리 안 되는 문제 메시지를 따로 모아두는 격리 보관함. 주소 불명 택배를 별도 창고에 두고 정상 처리는 계속 흐르게 하는 것과 같다.
- **replay / discard (재처리·폐기)** — replay 는 격리된 실패 메시지를 원래 줄로 다시 보내 재처리, discard 는 더 손대지 않기로 하는 것. 둘 다 누가·왜 했는지 기록(audit)을 남긴다.
- **dry-run (예행연습)** — 진짜로 실행하기 전에 '이렇게 하면 몇 건이 영향받는지'만 먼저 보여주는 미리보기. 운영자가 범위를 확인한 뒤에야 실제 실행(confirm)으로 넘어간다.
- **soft delete (소프트 삭제)** — 진짜로 지우지 않고 '폐기됨' 표시만 남겨 추적 기록을 보존하는 것. 실제 삭제는 보관 기간(retention)이 지난 뒤에만 일어난다.
- **fail-closed (닫은 채 실패)** — 보조 장치(Redis)가 죽으면 막아버리는 안전 우선 정책. 운영자의 대량 작업은 실수의 파장이 커서, 일반 사용자(fail-open)와 반대로 잠그는 쪽을 택했다.
- **scope (스코프, 권한 범위)** — 'DLQ 읽기만'·'재처리까지'·'대량 작업까지'처럼 작업별로 따로 떼어둔 권한 등급. 위험한 작업일수록 더 높은 등급과 더 낮은 호출 한도를 건다.
