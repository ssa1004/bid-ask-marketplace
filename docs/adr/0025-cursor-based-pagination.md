...
- **cursor pagination (커서 페이지네이션)** — '몇 페이지'가 아니라 '직전에 본 마지막 항목 다음부터' 가져오는 방식. 책갈피를 꽂아 거기서부터 이어 읽기라, 데이터가 쌓여도 항상 빠르다.
- **OFFSET 뒷페이지 절벽** — '100000번째부터 20개'를 달라고 하면 DB 가 앞의 10만 개를 다 읽고 버린 뒤 20개만 주는 비효율. 페이지가 뒤로 갈수록 점점 느려진다.
- **opaque token (불투명 커서)** — 커서 안에 든 위치 정보를 클라이언트가 못 읽게 Base64 로 가린 값. 그냥 다음 요청에 되돌려주기만 하면 돼, 나중에 서버가 커서 형식을 바꿔도 클라이언트는 안 깨진다.
- **tie-breaker (동점 가르기)** — 같은 시각에 만들어진 두 항목의 순서를 정하기 위해 id 까지 함께 비교하는 것. 시각만 보면 순서가 모호해 항목이 두 번 보이거나 누락될 수 있다.
