...
- **transient 오류 (일시적 오류)** — 잠깐 생겼다 금방 사라지는 오류. 네트워크가 순간 끊기거나 서버가 잠시 바쁜 경우라, 한두 번 다시 시도하면 대개 성공한다.
- **exponential backoff (지수 백오프)** — 재시도 간격을 1→2→4→8초처럼 점점 두 배로 늘리는 것. 실패한 외부 서버에 같은 강도로 바로 다시 두드리지 않고 회복할 여유를 준다.
- **jitter (지터, 무작위 흔들기)** — 재시도 시점에 살짝 무작위를 섞어 흩뜨리는 것. 안 섞으면 실패한 호출자들이 똑같은 순간 다시 몰려(thundering herd) 회복 중인 서버를 또 무너뜨린다.
- **thundering herd (썬더링 허드)** — 외부가 회복하는 순간 모두가 동시에 재시도를 던져 다시 짓밟는 현상. 넘어진 사람에게 모두 동시에 손을 뻗으면 더 깔리는 것과 같다.
- **화이트리스트 retry (5xx만, 4xx는 제외)** — '서버 탓(5xx)·네트워크 문제'는 재시도하고, '내 요청이 잘못된 것(4xx)'은 재시도해도 같은 결과라 절대 안 하는 정책.
