...
- **2단 캐시 (L1/L2)** — 캐시를 2층으로 둔 것. L1 은 서버 안 메모리(아주 빠름), L2 는 Redis(여러 서버가 공유). 가까운 서랍(L1)→공용 창고(L2)→공장 주문(DB) 순으로 찾는다.
- **cache stampede / thundering herd (캐시 쇄도)** — 캐시 유효기간이 끝나는 그 순간 여러 요청이 동시에 '없네' 하며 DB 로 우르르 몰려 DB 를 짓밟는 현상. 셔터 올라가는 순간 손님이 한꺼번에 몰리는 것과 같다.
- **cache-aside (캐시 곁두기)** — 먼저 캐시를 보고 없으면 DB 에서 읽어와 캐시에 채워두는 가장 흔한 방식. 단순하지만 만료 순간 쇄도에 약하다.
- **XFetch (확률적 조기 갱신)** — 캐시가 만료되기 살짝 전에, 무거운 항목일수록·확률적으로 미리 한 명만 갱신해 두는 기법. 모두가 만료 순간에 동시에 몰리지 않게 시점을 흩뜨린다.
- **fail-open (열어둔 채 실패)** — 보조 장치(여기선 Redis 캐시)가 고장 나면 막지 말고 그냥 통과시키는 정책. 캐시는 속도용 보조일 뿐 정확성은 DB 가 따로 지키니, 멈추기보다 느려도 돌아가게 한다.
