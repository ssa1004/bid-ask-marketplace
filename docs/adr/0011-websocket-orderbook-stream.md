...
- **fan-out (팬아웃)** — 한 번 일어난 변화를 그걸 보고 있는 모든 사람에게 한꺼번에 쫙 뿌리는 것. 선풍기 날개(fan)처럼 하나가 여러 갈래로 퍼진다.
- **consumer group (컨슈머 그룹)** — Kafka 에서 메시지를 나눠 받는 팀 이름. 같은 그룹이면 메시지를 한 명만 받고, 그룹을 따로 두면 각 그룹이 같은 메시지를 다 받는다. 인스턴스마다 그룹을 따로 둬서 모두가 같은 변경 알림을 받게 했다.
