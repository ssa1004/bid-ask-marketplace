...
- **Saga / 보상 트랜잭션 (compensation)** — 여러 단계로 이뤄진 긴 거래를 하나로 묶지 못할 때, 중간에 실패하면 앞서 한 일을 반대 동작으로 되돌리는 것. 이미 보낸 돈은 무를 수 없으니 '환불'이라는 반대 거래를 새로 한다.
- **멱등성 (idempotency)** — 같은 요청이 두 번 와도 결과가 한 번 한 것과 똑같게. 환불 신호가 두 번 들어와도 돈은 한 번만 돌려주게 만든다.
- **at-least-once (적어도 한 번)** — 메시지가 빠짐없이 가도록 하다 보니 가끔 두 번 올 수도 있는 보장. 안 오는 것보단 두 번 오는 게 낫다는 선택이라, 받는 쪽이 중복을 견뎌야 한다.
- **REQUIRES_NEW (별도 트랜잭션)** — 어떤 기록을 본 작업과 분리된 '별도 거래'로 떼어 박는 설정. 본 작업이 실패해 되돌려져도 '외부 호출을 시도한 흔적'은 따로 남아, 다음 재시도가 중복 호출을 피한다.
- **external id (외부 식별자)** — PG 의 환불번호·은행의 송금번호처럼 외부 시스템이 돌려준 영수증 번호. 이걸 장부에 적어두면 재시도 때 외부를 다시 부르지 않고도 결과를 알 수 있다.
