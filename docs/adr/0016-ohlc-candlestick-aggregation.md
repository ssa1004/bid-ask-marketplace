...
- **bucket / alignment (버킷·시간 정렬)** — 시간을 1분·5분 같은 일정한 칸으로 나눠 담는 것이 bucket, 14:23:45 를 14:23:00 처럼 칸의 시작 시각으로 딱 맞추는 게 alignment. 같은 시각이면 누가 계산해도 항상 같은 칸에 들어간다.
- **DB visibility (가시성)** — 한 트랜잭션이 막 저장(commit)한 데이터가 다른 트랜잭션 눈에 보이기까지의 찰나의 시차. 방금 들어온 체결이 집계 배치 눈에 아직 안 보일 수 있어, 배치를 5초 미뤄 안전하게 보이게 한 뒤 돌린다.
