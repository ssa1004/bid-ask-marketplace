...
- **Snowflake ID (스노우플레이크 ID)** — 시간 순으로 정렬되는 64비트 숫자 ID. 안에 '발급 시각 + 어느 서버 + 그 순번'이 박혀 있어 숫자만 비교해도 누가 먼저 생겼는지 안다. UUID 와 달리 가볍고(8바이트) 시간순이라 차트·무한스크롤에 딱.
- **write amplification (쓰기 증폭)** — 무작위 UUID 라 INSERT 할 때마다 인덱스의 여기저기 흩어진 곳에 써야 해서 실제 디스크 쓰기가 불어나는 현상. 시간순 ID 는 항상 맨 뒤에 차곡차곡 붙어 이 낭비가 사라진다.
- **buffer pool / page cache (버퍼 풀·페이지 캐시)** — DB 가 자주 쓰는 데이터를 디스크 대신 메모리에 올려두는 '자주 쓰는 서랍'. 인덱스가 가벼울수록 더 많이 올라가 조회가 빨라진다.
- **clock backward / NTP (시계 되돌림)** — 서버 시계가 자동 동기화(NTP) 중 아주 살짝 뒤로 가는 일. 이때 과거 시각으로 ID 를 또 만들면 충돌 위험이라, 작은 되돌림은 순번만 늘려 버티고 큰 되돌림은 즉시 멈춘다.
- **fail-fast (빠른 실패)** — 뭔가 잘못됐으면 조용히 망가진 채로 굴러가지 말고 즉시 에러로 멈추는 것. 잘못된 데이터가 쌓여 나중에 더 크게 터지는 걸 막는다.
- **RTT (왕복 시간, Round-Trip Time)** — 요청을 보내고 답이 돌아오기까지의 시간. DB sequence 는 매번 DB 에 왕복해 ID 를 받아야 해서 느리지만, Snowflake 는 앱 안에서 바로 만들어 왕복이 없다.
